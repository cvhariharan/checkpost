<script lang="ts">
  ...
  async function loadRules() {
    ...
    error = ''
    try {
      const data = await fetchAlertRules({ page: rulePage, countPerPage })
      rules = data.rules || []
  ...
  async function loadTargets() {
    ...
    error = ''
    try {
      const data = await fetchAlertTargets({ page: targetPage, countPerPage })
...
<section class="vstack gap-4">
  <header class="mb-4">
    ...
    <p class="text-light">Rules that watch the fleet and the targets they notify</p>
  </header>
  ...
  <ot-tabs bind:this={tabs} class="alerts-tabs" onot-tab-change={handleTabChange}>
      ...
      <div class="vstack gap-3">
        <div class="hstack justify-between">
          <div>
            ...
            <p class="text-light">Conditions evaluated against the fleet</p>
          </div>
          {#if canCreateRule}
            <button type="button" onclick={openCreateRule}>Create Rule</button>
          {/if}
        </div>
        ...
        {:else}
            ...
            <table>
              ...
              <tbody>
                {#each rules as rule}
                  <tr>
                    <td>
                      <button type="button" class="cell-link" onclick={() => openEditRule(rule)}>
                        {rule.name || 'Untitled'}
                      </button>
                    ...
                    <td class="align-right">
                      {#if canUpdateRule || canDeleteRule}
                        <ActionsMenu label={`Actions for ${rule.name || 'rule'}`}>
                          {#if canUpdateRule}
                            <button role="menuitem" type="button" onclick={() => openEditRule(rule)}>Edit</button>
                          {/if}
                          {#if canUpdateRule && canDeleteRule}<hr />{/if}
                          {#if canDeleteRule}
                            <button role="menuitem" type="button" onclick={() => confirmDeleteRule(rule)}>Delete</button>
                          {/if}
                        </ActionsMenu>
                      {/if}
                    </td>
          ...
          <footer class="hstack justify-between">
            <p class="text-light"><strong>{ruleTotalCount}</strong> rules</p>
            <Pagination currentPage={rulePage} pageCount={rulePageCount} onPageChange={changeRulePage} />
          </footer>
        {/if}
      </div>
    </div>
    ...
    <div role="tabpanel">
      <div class="vstack gap-3">
        <div class="hstack justify-between">
          ...
          {#if canCreateTarget}
            <button type="button" onclick={openCreateTarget}>Create Target</button>
          {/if}
        </div>
        ...
        {:else}
            ...
            <table>
              ...
              <tbody>
                {#each targets as target}
                  <tr>
                    <td>
                      <button type="button" class="cell-link" onclick={() => openEditTarget(target)}>
                    ...
                    <td class="align-right">
                      {#if canUpdateTarget || canDeleteTarget || canTestTarget}
                        <ActionsMenu label={`Actions for ${target.name || 'target'}`}>
                          ...
                          {/if}
                          {#if canUpdateTarget}
                            <button role="menuitem" type="button" onclick={() => openEditTarget(target)}>Edit</button>
                          {/if}
                          {#if canDeleteTarget}<hr />
                            <button role="menuitem" type="button" onclick={() => confirmDeleteTarget(target)}>Delete</button>
                          {/if}
                        </ActionsMenu>
                      {/if}
                    </td>
          ...
          <footer class="hstack justify-between">
            <p class="text-light"><strong>{targetTotalCount}</strong> targets</p>
            <Pagination currentPage={targetPage} pageCount={targetPageCount} onPageChange={changeTargetPage} />
          </footer>
        {/if}
      </div>
    </div>
