...
func (q *Queries) GetQuerySchema(ctx context.Context, arg GetQuerySchemaParams) (GetQuerySchemaRow, error) {
	row := q.db.QueryRowContext(ctx, getQuerySchema, arg.SourceUuid, arg.SqlVersion)
	var i GetQuerySchemaRow
	err := row.Scan(
		&i.SourceUuid,
		&i.SqlVersion,
		&i.Columns,
...
type ListQuerySchemasForSourceRow struct {
	SourceUuid       uuid.UUID       `db:"source_uuid" json:"source_uuid"`
	SqlVersion       int32           `db:"sql_version" json:"sql_version"`
	Columns          json.RawMessage `db:"columns" json:"columns"`
	FirstObservedAt  time.Time       `db:"first_observed_at" json:"first_observed_at"`
...
func (q *Queries) ListQuerySchemasForSource(ctx context.Context, sourceUuid uuid.UUID) ([]ListQuerySchemasForSourceRow, error) {
	rows, err := q.db.QueryContext(ctx, listQuerySchemasForSource, sourceUuid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListQuerySchemasForSourceRow
	for rows.Next() {
		var i ListQuerySchemasForSourceRow
		if err := rows.Scan(
			&i.SourceUuid,
			&i.SqlVersion,
			&i.Columns,
...
type UpsertQuerySchemaRow struct {
	SourceUuid       uuid.UUID       `db:"source_uuid" json:"source_uuid"`
	SqlVersion       int32           `db:"sql_version" json:"sql_version"`
	Columns          json.RawMessage `db:"columns" json:"columns"`
	FirstObservedAt  time.Time       `db:"first_observed_at" json:"first_observed_at"`
...
func (q *Queries) UpsertQuerySchema(ctx context.Context, arg UpsertQuerySchemaParams) (UpsertQuerySchemaRow, error) {
