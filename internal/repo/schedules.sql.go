...
const updateSystemScheduleInterval = `-- name: UpdateSystemScheduleInterval :one
UPDATE schedules SET
    interval_seconds = $2,
    updated_at = now()
...
type UpdateSystemScheduleIntervalParams struct {
	Uuid            uuid.UUID `db:"uuid" json:"uuid"`
	IntervalSeconds int32     `db:"interval_seconds" json:"interval_seconds"`
}

func (q *Queries) UpdateSystemScheduleInterval(ctx context.Context, arg UpdateSystemScheduleIntervalParams) (Schedule, error) {
	row := q.db.QueryRowContext(ctx, updateSystemScheduleInterval, arg.Uuid, arg.IntervalSeconds)
	var i Schedule
	err := row.Scan(
		&i.ID,
		&i.Uuid,
