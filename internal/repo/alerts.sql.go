...
const listDueAlertRules = `-- name: ListDueAlertRules :many
SELECT id, uuid, name, description, source, params, severity, enabled, evaluation_interval_seconds, for_seconds, repeat_interval_seconds, last_evaluated_at, created_at, updated_at FROM alert_rules
WHERE enabled = true
  AND (last_evaluated_at IS NULL
       OR now() - last_evaluated_at >= make_interval(secs => evaluation_interval_seconds))
`

func (q *Queries) ListDueAlertRules(ctx context.Context) ([]AlertRule, error) {
	rows, err := q.db.QueryContext(ctx, listDueAlertRules)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateAlertRuleEvaluatedAt = `-- name: UpdateAlertRuleEvaluatedAt :exec
UPDATE alert_rules SET last_evaluated_at = now() WHERE id = $1
`

func (q *Queries) UpdateAlertRuleEvaluatedAt(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, updateAlertRuleEvaluatedAt, id)
	return err
}
