...
func runServer(flags *rootFlags) error {
	...
	api.GET("/machines/:id/metrics", h.HandleMachineMetrics, h.AuthorizeOwnedMachineView())
	api.GET("/metrics/schemas", h.HandleMetricSchemas, h.AuthorizeMachineOverviewSupportView())
	api.POST("/machines/:id/queries", h.HandleExecuteMachineQuery, h.Authorize(core.ResourceMachine, core.ActionExecute))
	api.GET("/machines/:id", h.HandleGetMachine, h.AuthorizeOwnedMachineView())
